...
class StringProcessingTest(unittest.TestCase):
    ...
    def test_dont_condense_whitespace(self):
        ...
        p1 = fuzzywuzzy.StringProcessor.replace_non_letters_non_numbers_with_whitespace(s1)
        p2 = fuzzywuzzy.StringProcessor.replace_non_letters_non_numbers_with_whitespace(s2)
...
class est(unittest.TestCase):
    ...
    def test_asciionly(self):
        for s in self.mixed_strings:
            ...
            s = fuzzywuzzy.asciidammit(s)
            fuzzywuzzy.asciionly(s)
    ...
    def test_fullProcessForceAscii(self):
        for s in self.mixed_strings:
            fuzzywuzzy.full_process(s, force_ascii=True)
...
class RatioTest(unittest.TestCase):
    ...
    def testPartialTokenSortRatio(self):
        self.assertEqual(fuzzywuzzy.partial_token_sort_ratio(self.s1, self.s1a), 100)
        self.assertEqual(fuzzywuzzy.partial_token_sort_ratio(self.s4, self.s5), 100)
    ...
    def testPartialTokenSetRatio(self):
        self.assertEqual(fuzzywuzzy.partial_token_set_ratio(self.s4, self.s7), 100)
    ...
    def testWRatioEqual(self):
        self.assertEqual(fuzzywuzzy.WRatio(self.s1, self.s1a), 100)
    ...
    def testWRatioPartialMatch(self):
        ...
        self.assertEqual(fuzzywuzzy.WRatio(self.s1, self.s3), 90)
    ...
    def testTokenSetForceAscii(self):
        ...
        score = fuzzywuzzy._token_set(s1, s2, force_ascii=True)
        self.assertEqual(score, 100)

        score = fuzzywuzzy._token_set(s1, s2, force_ascii=False)
        self.assertLess(score, 100)
...
class ValidatorTest(unittest.TestCase):
    ...
    def testCheckForNone(self):
        ...
        decorated_func = fuzzywuzzy.check_for_none(self.testFunc)
...
class ProcessTest(unittest.TestCase):
    ...
    def testWithProcessor(self):
        ...
        best = fuzzywuzzy.extractOne(query, events, processor=lambda event: event[0])
    ...
    def testWithCutoff(self):
        ...
        best = fuzzywuzzy.extractOne(query, choices, score_cutoff=50)
    ...
    def test_dedupe(self):
        ...
        result = fuzzywuzzy.dedupe(contains_dupes)
